...
class GeneralClient:
    def __init__(self, args, client_id, model, data_path, output_dir, override_args, is_shifted, seq_length=128, dtype=np.uint16):
        ...
        self.local_data_train = np.memmap(self.local_train_data_path, dtype=dtype, mode='r')
        self.num_train_samples = self.local_data_train.size
        self.local_data_test = np.memmap(self.local_test_data_path, dtype=dtype, mode='r')
        self.local_data_valid = np.memmap(self.local_valid_data_path, dtype=dtype, mode='r')
        self.output_dir = output_dir
        self.seq_length = seq_length
        self.is_shifted = is_shifted
        self.args = args
        self.device = override_args['device']
        self.model = model(override_args)
        self.local_output_dir = os.path.join(self.output_dir, "trainer_saved", "local_output_{}".format(self.client_id))
        self.step = 0
        self.router_step = 0
        self.expert_optimizer, self.router_optimizer = (None, None)
    ...
    def synchronize(self, server_model, collaboration_strategy, aggregation_strategy, client_rank):
        for name, client_param in self.model.named_parameters():
            if collaboration_strategy.is_global_model(name):
                ...
                client_dims = client_param.shape
                if len(client_dims) < 2:
                    breakpoint()
                ...
                with torch.no_grad():
                    ...
                    if client_param.shape != update_param.shape:
                        print("DEBUG: pruned for updated")
                        self.pruned[name] = (update_height, update_width)

    def train(self, local_num_steps, acc_steps=4, is_train_on_val=False):
        type_ctx = nullcontext() if self.device == 'cpu' else torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16)
        ...
        if self.expert_optimizer is None:
            ...
            learning_rate_scaling=self.args.learning_rate_scale)
        ...
        for step in tqdm(range(local_num_steps)):  
            ...
            for _ in range(acc_steps): # gradient accumulation
                if is_train_on_val:
                    x, y = get_batch(self.local_data_valid, self.seq_length, batch_size=self.args.micro_batch_size, is_shifted=self.is_shifted, device=self.device)
                else:
                    x, y = get_batch(self.local_data_train, self.seq_length, batch_size=self.args.micro_batch_size, is_shifted=self.is_shifted, device=self.device)
                with type_ctx:
            ...
            if len(self.pruned) > 0:
                print("DEBUG: handling pruned params")
    ...
    def eval( self, max_num_batches = 100 ):
        ...
        for _ in tqdm(range(max_num_batches)): 
            x, y = get_batch(self.local_data_test, self.seq_length, batch_size=self.args.micro_batch_size, is_shifted=self.is_shifted, device=self.device)
            with type_ctx:
