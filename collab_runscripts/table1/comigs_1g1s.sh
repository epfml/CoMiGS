...
for seed in ${seed_list[@]}; do
  ...
  -out_dir $OUT_DIR \
  -data_path $DATA_PATH \
  -wandb_run_name "1g1s_rerun_agnews_num_grounds_${ground}_num_steps_${num_steps}_seed_${seed}_exp0_${exp0_importance}_r_${r}_update_every_${update_router_every}" \
  -seed $seed \
  -eval_every 20 \
  -log_every 5 \
