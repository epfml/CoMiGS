...
for pruning_strength in ${pruning_strengths[@]}; do
  for seed in ${seed_list[@]}; do
    for p_lam in ${p_lam_list[@]}; do
      ...
      -out_dir $OUT_DIR \
      -data_path $DATA_PATH \
      -wandb_run_name "16161616_hetlora_agnews_num_grounds_${ground}_num_steps_${num_steps}_seed_${seed}_plam_${p_lam}_exp0_${exp0_importance}_r_${r}_update_every_${update_router_every}_ps_${pruning_strength}" \
      -seed $seed \
      -eval_every 20 \
      -log_every 5 \
